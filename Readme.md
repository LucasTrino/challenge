...
Para o proximo commit, eu pensei em adicionar os testes automatizados com **jest** para as funções criadas, ou adicionar algo em relação a programação defensiva. Acho que para melhorar o que já existe, vou para a segunda opção. Claro que começar pelos testes é sempre a melhor opção, mas dado o escopo reduzido do desafio e o tempo, alem das configurações de setup para isso, talvez partir para melhorar a funcionalidade das funções, em busca de melhorar a consistencia, eu vou adicionar mais valor para a entrega, e aplicando uma programação defensiva eu já aumento a segurança e confiabilidade das funções.

---

#### commit-6c8e92d:

Nesse commit eu apliquei algumas estrategias da programação defensiva na 'função principal': 'findAnagrams()'. Adicionei um bloco **try/catch** e coloquei alguns comentarios em cada alteração, e vou esclarecer cada uma abaixo:

- A primeira foi testar se a o parsed JSON me volta, de fato, um array:
  `    
...
* O terceiro foi garantir que as chaves, que precisamos para operar sobre o JSON, são consistentes, ou seja, teem o mesmo 'nome':
`   
...
![defensive 3](./imgs/defensive%204.png)
